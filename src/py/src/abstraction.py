...
class Holding:
    def __init__(self):
        ...
        self._name_hold = HyprWorkspace.name_from_special("HOLD")

    def push(self, window_curr: typing.Optional[HyprWindow] = None) -> None:
        window_curr = window_curr or HyprWindow.from_current()
    ...
    def window_previous_non_hold() -> HyprWindow:
        windows = HyprWindow.windows(sort_by_focus=True)
        next(windows)  # pop the first (current) window
    ...
    def select_multi(self) -> cabc.Generator[HyprWindow, None, None]:
    ...
    def select(self) -> HyprWindow:
        ...
        return HyprWindow.from_address(self._pattern.match(choice).group(1))

    def _choices(self) -> cabc.Sequence[str]:
        return [w.selection_prompt() for w in self._windows()]

    def _windows(self) -> cabc.Generator[HyprWindow, None, None]:
        for j in HyprWindow.windows_json(sort_by_focus=True):
            if j["workspace"]["name"] == self._name_hold:
                yield HyprWindow.from_json(j)
